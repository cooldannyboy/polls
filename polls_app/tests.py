...
class QuestionViewTests(TestCase):
    ...
    def test_index_view_with_a_past_question(self):
        ...
        c=Choice.objects.create(question = q, choice_text="choice1")
        response = self.client.get(reverse('polls_app:index'))
    ...
    def test_index_view_with_future_question_and_past_question(self):
        ...
        c=Choice.objects.create(question = q1, choice_text="choice1")
        q2=create_question(question_text="Future question.", days=30)
        c=Choice.objects.create(question = q2, choice_text="choice1")
        response = self.client.get(reverse('polls_app:index'))
        self.assertQuerysetEqual(
            response.context['latest_question_list'],
            ['<Question: Past question.>']
    ...
    def test_index_view_with_a_question_without_choice(self):
        """
        A question without any choice should return False
        :return:
        """
        no_choice_question = create_question(question_text='No choice Question.',
                                        days=-1)

        has_choice_question = create_question(question_text='Has choice Question.',
                                        days=-1)

        c = Choice.objects.create(question = has_choice_question, choice_text="choice1")

        response = self.client.get(reverse('polls_app:index'))
        self.assertQuerysetEqual(
            response.context['latest_question_list'],
            ['<Question: Has choice Question.>']
        )
...
class QuestionIndexDetailTests(TestCase):
    def test_detail_view_with_a_future_question(self):
        ...
        future_question = create_question(question_text='Future question.',
                                          days=5)
        response = self.client.get(reverse('polls_app:detail',
